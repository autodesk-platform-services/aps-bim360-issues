    ...
    public class DataManagementController : ControllerBase
    {
        ...
        private async Task<IList<jsTreeNode>> GetHubsAsync()
        {
            ...
            { 
                ...
                //only check BIM360 hub. filter out other types of hubs
                if((string)hubInfo.Value.attributes.extension.type == "hubs:autodesk.bim360:Account"){
                    nodeType = "bim360Hubs"; 
                     // create a treenode with the values
                    jsTreeNode hubNode = new jsTreeNode(hubInfo.Value.links.self.href, hubInfo.Value.attributes.name, nodeType,null , !(nodeType == "unsupported"));
                    nodes.Add(hubNode);
                }  
            }
        ...
        private async Task<IList<jsTreeNode>> GetProjectsAsync(string href)
        {
            ...
            foreach (KeyValuePair<string, dynamic> projectInfo in new DynamicDictionaryItems(projects.data))
            {
                ...
                {
                    //This sample only works with BIM360 projects
                ...
                //container id
                string containerId = (string)projectInfo.Value.relationships.issues.data.id;
                dynamic data = new JObject();
                data.containerId = containerId;
                data.projectId = projectInfo.Value.id;

                // create a treenode with the values
                jsTreeNode projectNode = new jsTreeNode(projectInfo.Value.links.self.href, projectInfo.Value.attributes.name, nodeType,data, true);
        ...
        private async Task<IList<jsTreeNode>> GetProjectContents(string href)
        {
            ...
            {
                dynamic data = new JObject(); 
                data.folderId = folder.Value.id; 
                nodes.Add(new jsTreeNode(folder.Value.links.self.href, folder.Value.attributes.displayName, "folders",data, true));
            }
            return nodes;
        }
        ...
        private async Task<IList<jsTreeNode>> GetFolderContents(string href)
        {
            ...
            foreach (KeyValuePair<string, dynamic> folderContentItem in folderData)
            {
                        ...
                        if (includedItem.Value.relationships.item.data.id.IndexOf(folderContentItem.Value.id) != -1)
                        {
                            ...
                            foreach (KeyValuePair<string, dynamic> folderContentItem1 in folderData)
                            {
                                ...
                                if (folderContentItem1.Value.attributes.extension.data.sourceFileName == includedItem.Value.attributes.extension.data.sourceFileName)
                                {
                                    ...
                                    dynamic data = new JObject(); 
                                    data.itemUrn = includedItem.Value.id;
                                    nodes.Add(new jsTreeNode(treeId, WebUtility.UrlDecode(includedItem.Value.attributes.name), "bim360documents",data,false));
                                }
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    dynamic data = new JObject();
                    data.itemUrn = folderContentItem.Value.id;
                    nodes.Add(new jsTreeNode(folderContentItem.Value.links.self.href, folderContentItem.Value.attributes.displayName, (string)folderContentItem.Value.type, data, true));
                }
            }
        ...
        private async Task<IList<jsTreeNode>> GetItemVersions(string href)
        {
            ...
            foreach (KeyValuePair<string, dynamic> version in new DynamicDictionaryItems(versions.data))
            {
                ...
                dynamic data = new JObject();
                data.versionUrn = version.Value.id;
                data.versionNum = version.Value.attributes.versionNumber;
        ...
        {
            public jsTreeNode(string id, string text, string type, dynamic data, bool children)
            {
                this.id = id;
                this.text = text;
                this.type = type;
